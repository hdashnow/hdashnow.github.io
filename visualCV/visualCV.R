...
VisualResume::VisualResume(
  titles.left = c("Harriet Dashnow", "Bioinformatician", 
                  "*Built with in R using the InfoResume package: www.ndphillips.github.io/inforesume"),
  ...
  interests = list("programming" = c(rep("R", 5), rep("Python", 10), rep("Bpipe", 2)),
                   "community" = c(rep("COMBINE", 10), rep("ABACBS", 5)),
                   "writing and teaching" = c(rep("Elegant SciPy", 10), rep("Software Carpentry", 20), rep("Data Carpentry", 5))
                   ),
  year.steps = 1
)
dev.off()